...
export default function RingRingDerby() {
  ...
  return (
    <div style={{
      ...
      <>
      {/* Header */}
      <div style={{ textAlign: "center", marginBottom: "10px", animation: "fadeIn 0.6s ease" }}>
        <div style={{ fontSize: "10px", letterSpacing: "5px", textTransform: "uppercase", color: "#bbb", fontWeight: 600, marginBottom: "4px" }}>
      ...
      {isAdmin && !SHEET_CSV_URL && (
        <>
          ...
          {showEditor && (
            <div style={{ maxWidth: "880px", margin: "0 auto 10px", padding: "16px", background: "#fafafa", borderRadius: "14px", border: "1px solid #eee", overflowX: "auto" }}>
          ...
          <div style={{ transform: "rotateX(14deg)", transformOrigin: "50% 50%", position: "relative" }}>
            <svg viewBox={`0 0 ${svgW} ${svgH}`} style={{ width: "100%", height: "auto", display: "block", borderRadius: "16px", overflow: "hidden" }}>
              ...
              {horsesWithPos.map(({ store, pos, i: idx, total }) => {
                ...
                const rank = rankedStores.findIndex(s => s.name === store.name);
                const rankBonus = total > 0 ? Math.max(0, (16 - rank) * 0.8) : 0;
                const emojiSize = Math.max(24, 38 * depthScale + rankBonus);
      ...
      <div style={{ maxWidth: "420px", margin: "12px auto 0" }}>
        <button onClick={() => setStandingsOpen(!standingsOpen)} style={{
          ...
          <span style={{ fontSize: "13px", letterSpacing: "3px", textTransform: "uppercase", color: "#999", fontWeight: 700 }}>
            🏆 Standings
          </span>
              ...
              return (
                <div key={store.name} className="standing-row" style={{
                  ...
                  <span style={{
                    ...
                    color: i === 0 ? "#6B7D45" : i === 1 ? "#9CA3AF" : i === 2 ? "#8A9A62" : "#ddd",
                  }}>{i === 0 ? "👑" : i + 1}</span>
                  <span style={{ fontSize: "13px", textAlign: "center" }}>{store.region === "East" ? "🌴" : "🏔"}</span>
                  <div style={{
                    fontSize: "13px", fontWeight: 800, color: store.silk,
